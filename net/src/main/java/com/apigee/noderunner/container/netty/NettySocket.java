...
{
    protected static final Logger log = LoggerFactory.getLogger(NettySocket.class);
    ...
    private void initializeInternal(String host, int port, Function listener)
    {
        ...
        ChannelFuture future =
            NettyFactory.get().connect(port, host, localAddress, new ChannelInitializer<SocketChannel>()
            {
                ...
                {
                    c.pipeline().addLast(new Handler(NettySocket.this));
                }
            });
        future.addListener(new ChannelFutureListener()
        {
            ...
            public void operationComplete(ChannelFuture f)
            {
                if (f.isSuccess()) {
                    ...
                    initChannel();
                    NettySocket.this.runner.enqueueEvent(NettySocket.this, "connect", null);
                } else {
                    NettySocket.this.runner.enqueueEvent(NettySocket.this, "error",
                                                       new Object[] { NetSocketServer.makeError(f.cause(), "connect error",
    ...
    public static Object newSocket(Context cx, Object[] args, Function func, boolean isNew)
    {
        ...
        NettySocket ret = new NettySocket();
    ...
    public static void connect(Context cx, Scriptable thisObj, Object[] args, Function func)
    {
        ...
        ((NettySocket)thisObj).initializeInternal(host, port, callback);
    }
    ...
    protected boolean write(Context cx, Object[] args)
    {
        ...
        if (callback != null) {
            ...
            future.addListener(new ChannelFutureListener() {
                ...
                {
                    if (cb != null) {
                        runner.enqueueCallback(cb, NettySocket.this, NettySocket.this, null);
                    }
                }
            });
        }
    ...
    public static void setNoDelay(Context cx, Scriptable thisObj, Object[] args, Function func)
    {
        ...
        NettySocket sock = (NettySocket)thisObj;
    ...
    {
        NettySocket sock = (NettySocket)thisObj;
        InetSocketAddress addr = sock.channel.localAddress();
            ...
            runner.enqueueTask(new ScriptTask()
            {
                ...
                public void execute(Context cx, Scriptable scope)
                {
                    socket.fireEvent("error",
                                     new Object[]{NetSocketServer.makeError(t, "error",
                                                                      cx, scope)});
                }
            });
        }
